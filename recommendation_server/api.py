    ...
    if isinstance(doc, dict):
        for k, v in doc.items():
            if isinstance(v, ObjectId):
                ...
                doc[k] = [serialize_document(item) if isinstance(item, dict) else 
...
async def get_student_profiles():
    try:
        ...
        studentprofiles = list(db.studentprofiles.find().limit(5)) if "studentprofiles" in collections else []
        student_profile = list(db.studentProfile.find().limit(5)) if "studentProfile" in collections else []
        
        result = {
            "studentprofiles": [serialize_document(profile) for profile in studentprofiles],
            "studentProfile": [serialize_document(profile) for profile in student_profile],
...
async def get_recommendations(student_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Looking for student profile with userId (string): {student_id}")
            ...
            try:
                for collection_name in collections_to_check:
                    if collection_name in db.list_collection_names():
                        ...
                        student_profile = db[collection_name].find_one({"_id": ObjectId(student_id)})
                        if student_profile:
                            logger.info(f"Found student profile in {collection_name} by _id")
                            break
            except Exception as e:
        ...
        if student_profile:
            ...
            min_price = price_range.get("min", 0) if isinstance(price_range, dict) else 0
            max_price = price_range.get("max", 100000) if isinstance(price_range, dict) else 100000
            
            logger.info(f"Using preferences: type={preferred_property_type}, " +
            ...
            for listing in popular_listings:
                ...
                listing["score"] = 0.5 + (elo / 10000 * 0.3)  # Base 0.5 + up to 0.3 from ELO
        ...
        for listing in all_listings:
            ...
            if preferred_property_type:
                ...
                if listing_type and listing_type == preferred_property_type:
                    ...
                    if (isinstance(listing_type, str) and isinstance(preferred_property_type, str) and
                        (listing_type.lower() in preferred_property_type.lower() or 
                         preferred_property_type.lower() in listing_type.lower())):
            ...
            if preferred_areas:
                ...
                for area in preferred_areas:
                    ...
                    if (area_lower in listing_city or 
                        area_lower in listing_address or
                        area_lower in listing_province):
            ...
            if university:
                ...
                if listing_university and listing_university == university:
                    ...
                    match_reasons.append(f"Near your university: {university_distance}km")
                elif university_distance < 3:
                    scores["university_match"] = 0.9
                    match_reasons.append(f"Close to university: {university_distance}km")
            ...
            listing_result["matchReasons"] = match_reasons[:3] if match_reasons else ["Recommended listing"]
            
            # Debug scoring
            logger.debug(f"Listing {listing.get('propertyName')}: score={final_score}, components={scores}")
            
            scored_listings.append(listing_result)
        
        # Sort by score (descending) and return the top 6
        sorted_listings = sorted(scored_listings, key=lambda x: x["score"], reverse=True)
        top_listings = sorted_listings[:6]
        
        logger.info(f"Returning {len(top_listings)} recommendations (from {len(scored_listings)} evaluated)")
        return top_listings
